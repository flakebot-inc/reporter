...
LATEST_VERSION=$(aws s3 ls s3://get-flakebot/r/ | grep PRE | grep -v alpha | grep -v latest | sort | tail -n 1 | awk '{print $2}' | sed 's/.$//')
...
aws s3 cp s3://get-flakebot/r/$LATEST_VERSION s3://get-flakebot/r/latest --recursive
                
# Invalidate the CloudFront cache for the latest directory
aws cloudfront create-invalidation --distribution-id $1 --paths "/r/latest*"